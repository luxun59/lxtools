...
def write_raw_file(data_list,
                   files_dir,
                   resample=16000,
                   index=0,
                   total=1):
    logging.info('Processing {} {}/{}'.format(files_dir, index, total))
    read_time = 0.0
    save_time = 0.0
    write_time = 0.0
    ...
    for item in data_list:
        ...
        audio = (audio * (1 << 15)) #将浮点归一化范围转化为整数范围
        ...
        print("wav:",len(audio))
        torchaudio.save(wav_file,
                            audio,
                            resample,
                            format=suffix,
                            bits_per_sample=16)
        txt_file =os.path.join(files_dir, key + '.txt')

        with open(txt_file,'w') as f_txt_out:
            f_txt_out.write(txt)
            f_txt_out.close()
...
if __name__ == '__main__':
    ...
    for i, chunk in enumerate(chunks):
        ##创建输出子文件夹
        shards_raws_files_path = os.path.join(args.shards_dir,
                                '{}_{:09d}'.format(args.prefix, i))
        shards_raw_list.append(shards_raws_files_path)
        os.makedirs(shards_raws_files_path, exist_ok=True)
        pool.apply_async(
            write_raw_file,
            (chunk, no_segments, shards_raws_files_path, args.resample, i, num_chunks))

    pool.close()
    pool.join()

    with open(args.shards_list, 'w', encoding='utf8') as fout:
        for name in shards_raw_list:
            fout.write(name + '\n')
