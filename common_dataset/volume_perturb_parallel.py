...
def main():
    ...
    parser.add_argument('--input_text', default=None, help='输入文本文件（可选）')
    parser.add_argument('--output_dir', required=True, help='输出目录')
    parser.add_argument('--output_wavscp', default='wav_perturbed.scp', help='输出的 wav.scp 文件')
    parser.add_argument('--output_text', default=None, help='输出文本文件（可选）')
    parser.add_argument('--min_gain', type=float, default=0.8, help='最小音量增益因子（与 volume-factors 互斥）')
    parser.add_argument('--max_gain', type=float, default=1.2, help='最大音量增益因子（与 volume-factors 互斥）')
    parser.add_argument('--volume_factors', type=str, default=None, help='逗号分隔的增益值列表，例如: 0.8,1.0,1.2')
    parser.add_argument('--select_mode', choices=['random', 'round_robin'], default='random', help='volume-factors 模式下的选择方式')
    parser.add_argument('--seed', type=int, default=42, help='随机种子')
    parser.add_argument('--num_workers', type=int, default=10, help='线程数')
